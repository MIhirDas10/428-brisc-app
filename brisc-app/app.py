...
@st.cache_resource
def load_model():
    # IMPORTANT: match your checkpoint training channels
    model = UNetMultiTask(n_channels=1, n_classes=1).to(DEVICE)
    sd = torch.load("unet_multitask_best.pth", map_location=DEVICE)
...
if uploaded is not None:
    ...
    if cls_prob is not None:
        st.write(f"**Classification probability (tumor present):** `{cls_prob:.4f}`")
